...
export module User {
  ...
  const sessionObserver: Observable<any> = Observable.create(function (
  ...
  async function register (
    ...
    try {
      ...
      if (password != '') {
        ...
        encryptedSecret = Utils.symmetricEncryption.encrypt(
          keypair.secret(),
          pash
        )
      }
  ...
  export async function testTransfer (
    ...
    try {
      let { xdrs } = await XDRBuilder.pledge(
        ...
        options?.amount ? options.amount : undefined
      )
      const xdr = await XDRBuilder.signXDR(
        xdrs[0].xdr,
   ...
   */
  export async function getNiftronCreditBalance (
    publicKey?: string,
    test?: boolean
  ): Promise<NiftronAssetResponse | null> {
    try {
      ...
      try {
        sourceAccount = await server.loadAccount(
          publicKey ? publicKey : merchantKeypair.publicKey()
        )
      } catch (err) {
        try {
      ...
      sourceAccount.balances.forEach(function (balance) {
        if (
          ...
          balance.asset_code == 'NIFTRON'
        ) {
          response = <NiftronAssetResponse>{
            assetCode: balance.asset_code,
            balance: parseFloat(balance.balance),
            limit: parseFloat(balance.limit),
   ...
   */
  export async function getTokensByPublicKey (
    publicKey?: string,
    test?: boolean
  ...
  export async function activateLiveNetUser (
    ...
    try {
      ...
      const signedGoLiveXdr = await XDRBuilder.signXDR(
        goLiveXdr,
        merchantKeypair.secret()
      )
      //submit go live xdr
      ...
      const signedActivateXdr = await XDRBuilder.signXDR(
        activateXdr,
        activateUserModel.userKeypair.secret()
      )
      //submit activate xdr
